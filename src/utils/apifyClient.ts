...
export const scrapeLinkedInPosts = async (hashtag: string) => {
  try {
    ...
    console.error('Error scraping LinkedIn posts (from API):', error);
    throw new Error(`Error scraping LinkedIn posts: ${message}`);
  }
};
// Uncomment the following code if you want to use the ApifyClient directly instead of the API route
